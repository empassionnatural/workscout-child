...
<?php else :
	...
	$resume_submit_link = get_permalink( get_option( 'resume_manager_submit_resume_form_page_id' ) );
	?>
	<div class="notification notice closeable margin-bottom-40">
			
			<p><span><?php esc_html_e( 'Have an account? &nbsp;', 'workscout' ); ?></span></p>
			...
			<?php elseif ( $account_required ) : ?>
				...
				<?php echo apply_filters( 'submit_resume_form_login_required_message',  esc_html__( ' You must be signed in to submit a resume.', 'workscout' ) ); ?>
	...
	<?php
        ...
        if ( $account_required ) {
            echo do_shortcode('[custom_workscout_register_form redirect="'.$resume_submit_link.'" type="applicant" job_id="'.$job_id.'"]');
        }
